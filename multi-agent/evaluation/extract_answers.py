...
def get_answer_from_search_results(neo4j: Neo4jConnection, userQuery: str, labels: List[str]) -> Optional[str]:
    ...
    try:
        ...
        if search_results:
            print(f"🔍 Tìm thấy {len(search_results)} kết quả phù hợp cho câu hỏi: {userQuery[:50]}...")
            # Generate answer
            answer = neo4j.generate_answer(userQuery, search_results)
            return answer
        else:
            return None
    except Exception as e:
        print(f"⚠️ Lỗi khi tìm kiếm hoặc trả lời: {e}")
        return None
